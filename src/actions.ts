		...
		add_cue_at_position: {
			...
			options: [
				...
				},
				{
					...
					default: 'event',
					choices: [
						{ id: 'event', label: 'Current event position' },
			...
			callback: async (evt) => {
				...
				const positionMode = String(evt.options.positionMode ?? 'event') as	| 'event'	| 'player'	| 'custom'
			...
			options: [
				{
					...
					label: 'Resi Event',
					default: '',
					choices: self.getEventChoices(),
				},
			],
			callback: async (evt) => {
				await self.selectEvent(String(evt.options.eventId || ''))
			},
		},
		select_encoder: {
			name: 'Select encoder',
			options: [
				{
					...
					label: 'Encoder',
					default: '',
					choices: self.getEncoderChoices(),
				},
			],
			callback: async (evt) => {
				await self.selectEncoder(String(evt.options.encoderId || ''))
			},
		},

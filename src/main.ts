...
public async addCueFromPosition(opts: {
	...
	if (opts.positionMode === 'player') {
		...
		if (!p) throw new Error('Current player position is not available yet')
		if (!this.isValidHmsMs(p)) throw new Error(`Current player position is invalid: ${p}`)
		position = this.normalizeHmsMs(p)
	} else {
		const custom = (opts.customPosition ?? '').trim()
		if (!custom) throw new Error('Custom position is required')
		if (!this.isValidHmsMs(custom)) throw new Error('Custom position must be like HH:MM:SS or HH:MM:SS.mmm')

...
export class ModuleInstance extends InstanceBase {
	...
	private livingas1?: string
	private pollTimer?: NodeJS.Timeout
	private pollInFlight = false
	private lastLoginMs = 0
	private loginInFlight?: Promise<void>
	private customerId?: string
	private venues: ResiVenue[] = []
	private events: ResiEvent[] = []
	private playersByVenueUuid: Record<string, ResiPlayer[]> = {}

	private selectedEncoderId?: string
	private encoderChoices: Array<{ id: string; label: string }> = []
	private encoderEventPollTimer?: NodeJS.Timeout
	...
	private selectedPlayerHardwareId?: string

	private playerPollTimer?: NodeJS.Timeout
	private playerPollInFlight = false

	private currentEventId?: string
	private currentEventProfileId?: string

	private eventMediaStartEpochMs?: number
	...
	private getCurrentEncoderPositionMs(): number | undefined {
		if (!this.eventMediaStartEpochMs) return undefined
...
this.updateStatus(InstanceStatus.Ok)
	...
	private async resiLogin(username: string, password: string): Promise<void> {
		...
		if (!res.ok) {
			...
			if (j) detail = String(j?.message ?? j?.error ?? JSON.stringify(j))
		...
		const parts: string[] = []
		if (this.refreshToken) parts.push(`refreshToken=${this.refreshToken}`)
		if (this.livingas1) parts.push(`livingas1=${this.livingas1}`)
		this.cookieHeader = parts.join('; ')
	}

	private async getUsersMe(): Promise<any> {
...
if (!username || !password) {
	...
	public async refreshProfileNow(): Promise<void> {
...
public getEncoderChoices(): Array<{ id: string; label: string }> {
	const choices: Array<{ id: string; label: string }> = []

	for (const event of this.events) {
		if (!event.encoderId) continue
	...
	public getPlayerChoices(): Array<{ id: string; label: string }> {
		const choices: Array<{ id: string; label: string }> = []
	...
	getEventChoices(): { id: string; label: string }[] {
	return [
		{ id: '', label: '(none)' },
	...
	public async refreshListsNow(): Promise<void> {
		await this.refreshProfileSmart(false)
		await this.refreshVenuesAndPlayers()
	}

	public async selectPlayer(playerHardwareId: string): Promise<void> {
	...
	this.log(
		'info',
		`Selected event "${event.name}" (${this.currentEventId})`
	)
}

	private startPlayerPolling(): void {
		if (this.playerPollTimer) clearInterval(this.playerPollTimer)
...
if (!this.cookieHeader) return
			...
			if (!res.ok) throw new Error(`players poll failed (${res.status})`)

			const players = (await res.json()) as any[]
			const p = players.find((x: any) => x.playerHardwareId === this.selectedPlayerHardwareId)
...
if (!this.cookieHeader) return
		...
		if (!evRes.ok) throw new Error(`event detail failed (${evRes.status})`)

		const ev = (await evRes.json()) as { eventProfileId: string; startTime?: string; stopAfter?: string }
		...
		if (ev.startTime) {
			...
			if (!Number.isNaN(t)) this.eventMediaStartEpochMs = t
		}

		// cues
		const cuesRes = await fetch(
			`https://central.resi.io/api_v2.svc/streamprofiles/${this.currentEventProfileId}/events/${this.currentEventId}/cues?canISetCues=1`,
			{ method: 'GET', headers: this.buildAuthHeaders() }
		)
		if (!cuesRes.ok) throw new Error(`cues failed (${cuesRes.status})`)

		const cuesJson = (await cuesRes.json()) as ResiCue[]
...
this.cues = cuesJson
	...
	private async pollOnce(isManual: boolean): Promise<void> {
...
if (!this.cookieHeader) return
	...
	private parseTimeToMs(s: string): number {
		...
		if (![hh, mm, ss, ms].every((n) => Number.isFinite(n))) return 0
...
private formatTod(epochMs: number): string {
...
private computeAndSetCueVars(posMs: number): void {
	...
	if (!venuesRes.ok) throw new Error(`venues failed (${venuesRes.status})`)

	const venuesJson = (await venuesRes.json()) as ResiVenue[]
	this.venues = venuesJson
		// events
		const eventsRes = await fetch(
			`https://central.resi.io/api/v3/customers/${this.customerId}/events`,
			{
				method: 'GET',
				headers: this.buildAuthHeaders(),
			}
		...
		const eventsJson = (await eventsRes.json()) as ResiEvent[]
	...
	for (const v of this.venues) {
		...
		const playersJson = (await playersRes.json()) as any[]
		this.playersByVenueUuid[v.uuid] = playersJson.map((p: any) => ({
...
private async getSelectedEncoderStatus(): Promise<string | undefined> {
	if (!this.customerId || !this.selectedEncoderId) return undefined

	const url =
		`https://central.resi.io/api/v3/customers/${this.customerId}/encoders/status`
	...
	const encoders = (await res.json()) as Array<{
		uuid: string
		status?: string
	}>

	const encoder = encoders.find(
		(encoder) => encoder.uuid === this.selectedEncoderId
	)

	return encoder?.status
}

private async findActiveEventForEncoder(): Promise<void> {
	if (!this.customerId) return
	...
	if (encoderStatus !== 'started') {
		...
		this.checkFeedbacks(
			'encoder_stream_active',
			'encoder_event_active'
		)
		return
	}
	...
	try {
		...
		this.log(
			'info',
			`AUTO EVENT: Active event detected: "${activeEvent.name}" (${activeEvent.uuid})`
		)
		...
		this.log(
			'info',
			`AUTO EVENT: Loaded "${activeEvent.name}" with ${this.cues.length} cues`
		)
	} catch (err) {
		...
		this.log(
			'warn',
			`AUTO EVENT: Failed to find active event: ${msg}`
		)
	}
}
private startEncoderEventPolling(): void {
	if (this.encoderEventPollTimer) {
		clearInterval(this.encoderEventPollTimer)
		this.encoderEventPollTimer = undefined
	}

	if (!this.selectedEncoderId) return

	const poll = async () => {
		if (this.encoderEventPollInFlight) return
	...
	this.encoderEventPollTimer = setInterval(
		poll,
		10000
	)
}
	public lookupCueByName(queryRaw: string, mode: 'exact' | 'contains' = 'exact'): void {
...
private async refreshCuesIfDue(): Promise<void> {
	// Must have an event loaded
	if (!this.currentEventId || !this.currentEventProfileId) return

	// Don’t overlap cue refreshes
	if (this.cueRefreshInFlight) return
	...
	const now = Date.now()
	if (now - this.lastCueRefreshMs < intervalMs) return
 ...
 */
public async addCueFromPosition(opts: {
	name: string
	privateCue: boolean
	positionMode: 'event' | 'player' | 'custom'
	customPosition?: string
}): Promise<void> {
	const cueName = (opts.name ?? '').trim()
	if (!cueName) throw new Error('Cue name is required')
	...
	if (!user) throw new Error('Missing username (needed for cue user field)')
...
if (opts.positionMode === 'event') {
	...
	if (!this.isValidHmsMs(custom)) {
		throw new Error(
			'Custom position must be like HH:MM:SS or HH:MM:SS.mmm'
		)
	}
...
public async selectEncoder(encoderId: string): Promise<void> {
	...
	const encoderName =
		this.getEncoderChoices().find((choice) => choice.id === id)?.label ?? id
		this.setVariableValues({
...
public async deleteCueByName(opts: {
	name: string
	matchMode: 'exact' | 'contains'
	deleteMode: 'first' | 'all'
	onlyMine: boolean
}): Promise<void> {
	const query = (opts.name ?? '').trim()
	if (!query) throw new Error('Cue name is required')
	...
	const cuesAny = this.cues as any[]

	const matches = cuesAny.filter((c: any) => {
		const nameOk =
			opts.matchMode === 'contains' ? norm(c.name ?? '').includes(qn) : norm(c.name ?? '') === qn

		if (!nameOk) return false

		if (opts.onlyMine) {
			// works only if c.user exists
			if (typeof c.user !== 'string') return false
...
async destroy(): Promise<void> {
	if (this.pollTimer) clearInterval(this.pollTimer)
	if (this.playerPollTimer) clearInterval(this.playerPollTimer)
	if (this.encoderEventPollTimer) clearInterval(this.encoderEventPollTimer)
	if (this.streamElapsedTimer) clearInterval(this.streamElapsedTimer)
